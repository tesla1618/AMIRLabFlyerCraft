...
const Generator = () => {
  ...
  function addAuthor() {
    const authors = currentAuthorName.split(",");
    const filteredAuthors = authors.filter((author) => author.trim() !== "");
    setauthorNames((prevAuthors) => [...prevAuthors, ...filteredAuthors]);
  ...
  const deleteAuthor = (index) => {
    setauthorNames((previousAuthors) =>
      previousAuthors.filter((_, i) => i !== index)
    );
  };
  ...
  const downloadImage = async () => {
    try {
      ...
      const dataUrl = await htmlToImage.toPng(socialTemplate.current, {
        quality: 1.9,
      });
      ...
      if (error instanceof Event) {
        console.error(
          "Error during image generation. Ignoring event object:",
          error
        );
      } else {
      ...
      <div className="container mx-auto mt-10 mb-5 px-4">
        <div className="grid lg:grid-cols-2 md:grid-cols-2 gap-x-3 sm:grid-cols-1 gap-y-3">
          <div className="bg-base-100 shadow-md rounded-md p-10">
            ...
            <div className="py-5 flex flex-col gap-y-2">
              <input
                ...
                type="text"
                placeholder="Journal/Conference/Book Chapter Name"
              ...
              <input
                onChange={changeDP}
                type="file"
                className="file-input input-bordered focus:input-primary"
                accept="image/*"
              />
              <div className="">
                <div className="flex flex-row gap-x-2">
                  <input
                    ...
                    className={
                      !noAuthor
                        ? "input flex-1 w-3/5 input-bordered focus:input-primary "
                        : "input flex-1 w-1/5 input-error focus:input-primary"
                    }
                    value={currentAuthorName}
                    onChange={(e) => setcurrentAuthorName(e.target.value)}
                  />
                  <button
                    className="flex-2 w-2/5 btn bg-green-600  hover:bg-black text-white"
                    onClick={addAuthor}
                  >
                    Add
                  </button>
                  ...
                  <ul className="mt-3">
                    {authorNames.map((text, index) => (
                      <li className="mb-2 flex flex-row gap-x-2" key={index}>
                        <p className="shadow-md border rounded-lg px-4 py-3 flex-1 w-3/4">
                          {text}
                        </p>
                        <button
                          className="btn bg-red-600 text-white hover:bg-black text-white"
              ...
              <div className="flex flex-row gap-x-3">
                <input
                  type="checkbox"
                  checked={authorIndex}
                  onChange={() => setauthorIndex(!authorIndex)}
          ...
          <div>
            <div
              ref={socialTemplate}
              className="shadow-md p-10 generated-image-wrap"
            >
              ...
              <div className="generated-content">
                ...
                <div className="success-in mt-3">
                  ...
                  <div className="journal-name mt-2">
                    {jName ? jName : "Journal Name"}
                  </div>
                </div>
                  ...
                  <div className="flex-1 author-dp w-2/5">
                    <div className="avatar">
                      <div className="rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
                        {authorDp ? (
                          <img width={120} id="image-preview" />
                        ) : (
                          <img width={120} src={defaultAvatar} />
                        )}
                      </div>
                    </div>
                  </div>
                  <div className="flex-2 w-3/5">
                    <div className="paper-title">
                      ...
                      <p className="text-white">
                        {title
                          ? title
                          : "The title of the paper is prominently featured in this designated section for easy identification and reference"}
                      </p>
                    </div>
                        ...
                        {authorNames && authorNames.length > 0
                          ? authorNames.map((author, index) => (
                              ...
                              >
                                {author}
                                {authorIndex && (
                                  <sup className="ml-1">{index + 1}</sup>
                                )}
                              </div>
                            ))
